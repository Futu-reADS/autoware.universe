...
bool StopLineModule::modifyPathVelocity(PathWithLaneId * path, StopReason * stop_reason)
{
  ...
  *stop_reason = planning_utils::initializeStopReason(StopReason::STOP_LINE);
  ...
  stop_point_idx = stop_point->first;
  stop_pose = stop_point->second;
   ...
   * s---ego----------x--|--------g
   */
  stop_line_seg_idx = planning_utils::calcSegmentIndexFromPointIndex(
    path->points, stop_pose.position, stop_point_idx);
  const size_t current_seg_idx = findEgoSegmentIndex(path->points);
  signed_arc_dist_to_stop_point = motion_utils::calcSignedArcLength(
  ...
  // removed the below switch case as added the code in dezyne file
  /*
  switch (state_) {
    case State::APPROACH: {
      // Insert stop pose
